...
For this preliminary study, we evaluate the framework on four representative datasets selected from the two dataset groups: two flat-tag benchmarks requiring a constructed concept graph (ASSISTments 2009~\cite{feng2009assistments} and Algebra~\cite{stamper2010kddcup}), and two benchmarks providing a native concept structure (XES3G5M~\cite{liu2023xes3g5m} and Eedi~\cite{wang2021neurips}). We compare against the foundational BKT~\cite{corbett1994kt} and two widely adopted deep knowledge tracing baselines, DKT~\cite{piech2015dkt} and DKVMN~\cite{zhang2017dkvmn}. More recent state-of-the-art methods are reserved for the comprehensive evaluation described in Section~\ref{sec:eval}. The framework results are reported in Table~\ref{tab:prelim_results} as the mean over five-fold cross-validation after 200 training epochs.
