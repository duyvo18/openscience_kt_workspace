...
\begin{enumerate}
    ...
    \item~\emph{Pattern construction} builds several learning patterns using fixed pattern operators, so that each operator always carries the same pedagogical meaning and pattern structure is consistent across learners regardless of history length or distribution. A planned list of operators is: (i)~\emph{temporal pooling} pools the most recent interactions; (ii)~\emph{same-KC pooling} pools all interactions on the same KC; and (iii)~\emph{$\mathcal{G}$-aware pooling} pools interactions following the KC topology.

    \item~\emph{Pedagogical alignment} aligns the patterns with pedagogical principles. Crucially, this alignment does not directly infer mastery. Instead, each principle is realized as a \emph{rule-shaped} transform of the pattern's distributional descriptor: the functional form of the transform is fixed by the principle. It encodes the pedagogical constraint, while only its adjustment amplitude is learnable. A planned list of principles is: (i)~\emph{difficulty-response consistency} reweights each interaction's contribution to a pooled pattern by its difficulty, grounding the adjustment in item response theory; (ii)~\emph{KC-relation consistency} adjusts the overall pattern distribution according to relations between KCs read from $\mathcal{G}$ (e.g., prerequisite); (iii)~\emph{guessing and slipping}: when a pattern of the same KC and similar difficulty contains an outlier response (a correct answer amid errors, or vice versa), the outlier is treated as a possible sign of guessing or slipping and the pattern is adjusted to absorb this possibility.
...
We report a preliminary evaluation of the framework's initial implementation. The implementation includes all four framework modules as presented in Section~\ref{sec:framework}, but the specific components remain simplified and have not yet been individually validated or optimized. Consequently, the results should be interpreted as an initial feasibility study rather than evidence of the framework's final performance.

For this preliminary study, we evaluate the framework on four representative datasets selected from the two dataset groups: two flat-tag benchmarks requiring a constructed concept graph (ASSISTments 2009~\cite{feng2009assistments} and Algebra~\cite{stamper2010kddcup}), and two benchmarks providing a native concept structure (XES3G5M~\cite{liu2023xes3g5m} and Eedi~\cite{wang2021neurips}). We compare against the foundational BKT~\cite{corbett1994kt} and two widely adopted deep knowledge tracing baselines, DKT~\cite{piech2015dkt} and DKVMN~\cite{zhang2017dkvmn}. More recent state-of-the-art methods are reserved for the comprehensive evaluation described in Section~\ref{sec:eval}. The framework results are reported in Table~\ref{tab:prelim_results} as the mean $\pm$ standard deviation over five-fold cross-validation after 200 training epochs.

\begin{table*}[!htbp]
\centering
\caption{Preliminary AUC comparison on four representative datasets.}
\label{tab:prelim_results}
\begin{tabular}{lcccc}
\toprule
& \multicolumn{2}{c}{Flat-tag} & \multicolumn{2}{c}{Native structure} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Method & ASSISTments 2009~\cite{feng2009assistments} & Algebra~\cite{stamper2010kddcup} & XES3G5M~\cite{liu2023xes3g5m} & Eedi~\cite{wang2021neurips} \\
\midrule
BKT~\cite{corbett1994kt}  & 0.6700 & -- & -- & -- \\
DKT~\cite{piech2015dkt} & \textbf{0.7541} & \textbf{0.8149} & \underline{0.7852} & \underline{0.7689} \\
DKVMN~\cite{zhang2017dkvmn} & \underline{0.7473} & \underline{0.8054} & 0.7792 & 0.7673 \\
\textbf{Ours} & $0.7128 \pm 0.0080$ & $0.7818 \pm 0.0063$ & $\mathbf{0.8065 \pm 0.0027}$ & $\mathbf{0.7912 \pm 0.0003}$ \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize
The best result in each column is shown in \textbf{bold}, and the second best is \underline{underlined}. Established results for BKT, DKT, and DKVMN are taken from their pyKT implementation~\cite{zitao2022pykt}, while our proposal is reported as the mean $\pm$ standard deviation over five-fold cross-validation. All training and evaluation paradigms are consistent.
\end{flushleft}
\end{table*}

The preliminary results reveal a consistent difference between the two dataset groups. On the flat-tag benchmarks, where the concept graph must first be constructed, the current implementation remains below DKT and DKVMN by approximately 2.9\% to 5.5\% in relative AUC. In contrast, on datasets providing native concept structures, the framework achieves the best performance, outperforming both deep baselines by approximately 2.7\% to 3.5\% in relative AUC. Although these results are preliminary, they are consistent with the central motivation of the proposal: explicitly modeling structured concept relationships appears to provide greater benefit when reliable concept structures are available. A comprehensive evaluation against recent state-of-the-art KT methods, together with ablation studies and statistical significance analyses, is left to the planned evaluation presented in Section~\ref{sec:eval}.

% \begin{table*}[!htbp]
% \centering
% \caption{Summary of datasets and their concept structures.}
% \label{tab:datasets}
% \small
% \begin{tabular}{lrrrrl}
% \toprule
% \textbf{Dataset} & \textbf{\#Stud.} & \textbf{\#Ques.} & \textbf{\#KCs} & \textbf{\#Inter.} & \textbf{Concept structure} \\
% \midrule
% \multicolumn{6}{l}{\textit{With a native concept structure (knowledge graph)}} \\
% \midrule
% Junyi Academy~\cite{chang2015junyi} & 247,606 & 722 & 41 & 25.9M & Expert-defined prerequisite graph \\
% XES3G5M~\cite{liu2023xes3g5m} & 18,066 & 7,652 & 865 & 5.55M & Hierarchical KC tree (KC routes) \\
% Eedi / NIPS34~\cite{wang2021neurips} & 4,918 & 948 & 57 & 1.38M & Subject-topic hierarchy (taxonomy) \\
% \midrule
% \multicolumn{6}{l}{\textit{With flat KC tags only (graph must be constructed)}} \\
% \midrule
% ASSISTments2009~\cite{feng2009assistments} & 4,217 & 17,751 & 123 & 347K & --- \\
% ASSISTments2012~\cite{feng2009assistments} & 46,674 & 179,999 & 265 & 6.12M & --- \\
% ASSISTments2017~\cite{feng2009assistments} & 1,709 & 3,162 & 102 & 942K & --- \\
% Algebra 2005--06~\cite{stamper2010kddcup} & 574 & 173,113 & 112 & 809K & --- \\
% Bridge to Algebra~\cite{stamper2010kddcup} & 1,146 & 129,263 & 493 & 3.68M & --- \\
% \bottomrule
% \end{tabular}
% \end{table*}
...
\textbf{Datasets.} We plan to work in the mathematics domain, where the concept structure the pedagogical operators rely on is best established. Datasets are organized by whether that structure is available natively. Some benchmarks ship a genuine graph or tree, including Junyi Academy~\cite{chang2015junyi}, XES3G5M~\cite{liu2023xes3g5m}, and Eedi/NIPS34~\cite{wang2021neurips}. Others are widely used flat-tag benchmarks, i.e., ASSISTments~\cite{feng2009assistments} and Algebra/Bridge-to-Algebra~\cite{stamper2010kddcup}, which we retain for scale and comparability. For the flat-tag benchmarks, we construct a graph following the validated procedure of CMDKT~\cite{xia2025cmdkt} and treat it as a fixed input (Section~\ref{sec:formulation}).
